...
function getMagickPath() {
    const basePath = isDev
      ? path.join(__dirname, '..') // В разработке: корень проекта
      : process.resourcesPath;     // В сборке: папка resources

    const magickPath = path.join(basePath, 'vendor', 'imagemagick', 'magick');
...
function runMagick(args) {
  ...
  return new Promise((resolve, reject) => {
    ...
    command.on('close', (code) => {
      if (code === 0) {
        console.log(`[RUN_MAGICK] Команда выполнена успешно. Вывод: ${stdout}`);
        resolve(stdout);
      } else {
...
async function convertToGif(groupName, pngFilePaths, outputDir, maxKB, frameDelay, colorCount, ditherType) {
  ...
  if (sizeInKB > maxKB) {
    console.log(`[RESIZE] GIF слишком большой (${sizeInKB}KB > ${maxKB}KB). Изменяю размер...`);
